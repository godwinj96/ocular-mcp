...
This directory contains the research, conclusions, and implementation plan for **Phase 1** of Ocular: the **Vision MCP server**, originally scoped and built as stdio-client-to-cloud-stealth-browser-pool.

> **⚠️ Amended 2026-09-01 (`docs/Ocular_PRD_v0.2.md`):** the framing above describes the cloud path only. As of PRD v0.2, Ocular is **one product with two execution paths** — a local worker (a Go supervisor driving `chrome-headless-shell` directly, for localhost/dev-server/authenticated targets) alongside the cloud stealth-browser pool documented in this directory (for public, unauthenticated targets). Everything in these documents about the cloud path stands; where a document describes local stdio as a thin proxy to the cloud backend, that description is superseded — see `docs/Ocular_PRD_v0.2.md` and `docs/rules/13-local-worker-and-distribution.md` for the current local-worker architecture.
...
The original brief's headline tooling choice — **"Playwright over Puppeteer"** — is correct for the _ergonomics_ (isolated `BrowserContext`s, auto-waiting) but **wrong as the stealth transport** as of 2026. Anti-bot vendors now fingerprint _how the browser is driven_ (the automation protocol / CDP handshake shape), not just the browser's JS-layer fingerprint. Vanilla Playwright and even `rebrowser-playwright` sit at the _bottom_ of current Cloudflare benchmarks.
