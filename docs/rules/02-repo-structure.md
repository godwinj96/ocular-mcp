...
6. **A contract used by two or more packages is written once, in `shared`, and imported — never re-typed per package.** "Package boundaries are the primary boundary" (principle 1) means _no package imports another package's runtime code_; it has never meant "re-derive the same key format or defaulting logic independently is fine because the alternative is a cross-import." Those are different problems with different fixes. A contract is: a key/field shape, "missing means X" semantics, TTL/expiry math, a hash algorithm, a verification routine — anything where two independent copies can silently drift. `packages/dashboard/lib/quota-reader.ts` did exactly this, citing this section as the reason; it wasn't one, and the duplication was a real bug waiting to happen (a quota key format changed in one copy and not the other). See `quota.ts` and `schemas/heartbeat.schema.ts` in `shared` for the pattern: the pure logic (what a key looks like, what an empty result means) lives in `shared` and takes its I/O client as a parameter; each package supplies only its own client instance.

   What legitimately stays per-package is the **I/O driver**, never the logic built on top of it. `mcp-server` uses `pg`/`Pool` (long-lived process); `dashboard` uses `@neondatabase/serverless` (short-lived serverless functions) — see §8. That split is real and stays. It is not license to also duplicate the SQL, the row-shape mapping, or the business rule sitting on top of the driver call — only the driver choice itself is allowed to differ.

   **Quota and worker/audit ownership**, concretely, per the account-state-service decision: the dashboard owns the quota _schema_, the read/report surface (`quota-reader.ts`), and is the only writer of `workers`, `capture_counters`, and `audit_events`. Redis quota **enforcement** (reserve/decrement) stays in `mcp-server`'s process, in-line with the render path it gates — moving it behind a network hop would put a Vercel cold start in front of every paid capture for no ownership benefit, since the write side was never the part that drifted. `mcp-server`'s `/worker/heartbeat` route authenticates the caller (work it already does for every tool call) and forwards the validated body to the dashboard's `/api/internal/worker-heartbeat`, which does the actual write — a background heartbeat every 5 minutes is not the render path, so the extra hop costs nothing a user notices. See `mcp-server/src/internal/dashboard-client.ts` and `dashboard/app/api/internal/worker-heartbeat/route.ts`.
